...
class PropositionsController < ApplicationController
  ...
  def latest
    ...
    @propositions = Proposition.latest_proposed.with_users.paginate :per_page => 10, :page => params['page']
    render :action => 'index'
  end
  
  def voting
    @title = "Propuestas a votación"
    @propositions = Proposition.to_vote.with_users.paginate :per_page => 10, :page => params['page']
    render :action => 'index'
  end
end
